	  ...
	  .done(function(data){
		...
		system_namef0.forEach(function(e,index) {
			if(system_namef0[index]==sys_num){
			    ...
			    $("#curimsTpsContainer").append(curTpsAddHtml);
			    $("#totimsTpsContainer").append(totTpsAddHtml);
			}
		});
		
		drawPieChart(curSess, totSess, "mss-chart"); 
				...
				if( getPrevDateTime() < date[index]+" "+time[index] && getDateTime() > date[index]+" "+time[index]){
						switch(type[index]){
							case "REG" : 
								...
								break;
							case "SESS" : 
								$("#SESSstat").append("<span class='sys-txt-value'>"+succ_rate[index]+"%</span>");
								if(succ_rate[index] < T_th1 && att[index] > T_th5){
							      	  $("#SESSstat").parents(".ims-stat-panel").addClass("alarm-twinkle");
								}
								break;
							case "MC" : 
								$("#MCstat").append("<span class='sys-txt-value'>"+succ_rate[index]+"%</span>");
								if(succ_rate[index] < T_th2 && att[index] > T_th6){
							      	  $("#MCstat").parents(".ims-stat-panel").addClass("alarm-twinkle");
								}
								break;
				...
				for(var i=0; i<type_f2.length; i++){
					if(type_f2[i] == "ALARM" && sys_sub_name_f2[index] == sys_sub_name_f2[i] && date[index]+" "+time[index] > date[i]+" "+time[i]){ 
						...
						case "A1660":
							...
							break;
						case "A1663":
							$("#A1663").removeClass("alarm-twinkle");
							break;
						}
					}
				}
...
(function($){
	  ...
	  executeSetInterval(function(){
	      ajaxShowMSSDetail("/api/v1/mss-list/"+$("#inputCurrentSystem").attr("val"));
	    }, _PERIOD_);
