...
function ajaxShowSgwDetail(url){
  $.ajax({
    url: url,
    type: "GET"
  })
  .done(function(data){
	...
	//fallback(1, 3)
	//통계 임계치 설정 및 통계 출력
	var s_format_CSR= "<div class=\"col-sm-10 sgw-stat-panel\">"+
	"<div id=\"CSRstat\" class=\"sgw-stat-txt\">CSR 성공률 </div></div>";
	
	var s_format_MBR= "<div class=\"col-sm-10 sgw-stat-panel\">"+
...
(function($){
  ...
  executeSetInterval(function(){
      ajaxShowSgwDetail("/api/v1/sgw-list/"+$("#inputCurrentSystem").attr("val"));
    }, _PERIOD_);
